...
Single organization database implementation.
"""
...
class DuckDBManager(DatabaseManager):
    ...
    Single organization implementation with one database file.
    """
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Optional explicit database path. Defaults to 'data/openeasd.db'
        """
    ...
    def _migrate_domains_table(self):
        ...
        print("Migrating domains table schema...")
    ...
    def initialize(self) -> None:
        ...
        # Note: Indexes on scan_sessions disabled due to DuckDB UPDATE constraint issue
    ...
    def get_domains(self, limit: int = 20, offset: int = 0,
        ...
        if domain_name:
            where_conditions.append("d.domain = ?")
            params.append(domain_name)

        if domain_type:
            where_conditions.append("d.domain_type = ?")
            params.append(domain_type)

        if primary_only:
            where_conditions.append("d.is_primary = TRUE")

        where_clause = f"WHERE {' AND '.join(where_conditions)}" if where_conditions else ""

        # Query domains with all metadata
        # Calculate scan_count and findings_count dynamically with JOINs (fixes N+1 query problem)
        domains_query = f"""
            SELECT
                d.domain,
                d.domain_type,
                ...
                d.contact_email,
                d.scan_frequency,
                d.active_scan_enabled,
                COUNT(DISTINCT a.id) as findings_count
            FROM domains d
            LEFT JOIN scan_sessions s ON s.domains_scanned[1] = d.domain
            LEFT JOIN security_alerts a ON a.domain = d.domain
            {where_clause}
            GROUP BY d.domain, d.domain_type, d.is_primary, d.created_at,
                     d.updated_at, d.last_scanned_at, d.notes, d.tags,
                     d.contact_email, d.scan_frequency, d.active_scan_enabled
        ...
        count_query = f"SELECT COUNT(DISTINCT d.domain) FROM domains d {where_clause}"
        ...
        for row in results:
            ...
            domains.append({
                ...
                'findings_count': row[12]  # Now from JOIN instead of separate query
            })
    ...
    def add_subdomain_to_history(self, apex_domain: str, subdomain: str,
    ...
    def get_subdomain_history(self, apex_domain: str,
                                   limit: int = 100,
    ...
    def get_subdomain_changes(self, apex_domain: str, scan_id: str) -> Dict[str, Any]:
        """
    ...
    # Scan Management
    def create_scan_session(self, scan_type: str, domains: List[str], tool_name: Optional[str] = None) -> str:
        """
