...
def mock_db_manager():
    ...
    # Mock the async methods if your db_manager has any
    db_manager.store_findings = AsyncMock()
...
@pytest.mark.asyncio
async def test_analyze_scan_results_workflow(analysis_service, mock_db_manager):
    ...
    # The actual call is without await in the service, but the mock is async
...
@pytest.mark.asyncio
async def test_store_findings_no_db_manager():
    ...
    await service._store_findings("scan-id", [])
...
@patch('src.analysis.analysis_service.PortVulnerabilityDetector')
@pytest.mark.asyncio
async def test_analyze_scan_results_detector_error(mock_port_detector, mock_risk_scorer, mock_get_config, mock_config, mock_db_manager):
    ...
    result = await service.analyze_scan_results("scan_id", {})
...
@patch('src.analysis.analysis_service.PortVulnerabilityDetector')
@pytest.mark.asyncio
async def test_analyze_scan_results_scorer_error(mock_port_detector, mock_risk_scorer, mock_get_config, mock_config, mock_db_manager):
...
@patch('src.analysis.analysis_service.PortVulnerabilityDetector')
@pytest.mark.asyncio
async def test_full_analysis_failure(mock_port_detector, mock_risk_scorer, mock_get_config, mock_config, mock_db_manager):
